...
function learningProgressUserId() {
  try {
    ...
    if (u === 'potkan') return 'potkan';
  } catch (e) {}
  return 'seybo';
}
var _progressUid = learningProgressUserId();
const STORAGE_KEY = _progressUid === 'potkan' ? 'sy701_state_v1_potkan' : 'sy701_state_v1';
const STORAGE_TS_KEY = _progressUid === 'potkan' ? 'sy701_state_v1_potkan_ts' : 'sy701_state_v1_ts';
...
function applyTheme(mode) {
  ...
  if (meta) meta.setAttribute('content', mode === 'light' ? '#f5f7fb' : '#0b1220');
  ...
  if (btn) {
    btn.setAttribute('aria-label', mode === 'dark' ? 'Zapnúť light mode' : 'Zapnúť dark mode');
